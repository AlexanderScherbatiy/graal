        ...
        if (function.equals("FindClass") || function.equals("DefineClass")) {
            ...
            if (!advisor.shouldIgnore(lazyValue(forNameString), callerClassLazyValue)) {
                if (function.equals("FindClass")) {
                    configuration.getOrCreateType(predicate, forNameString);
        ...
        switch (function) {
            ...
            case "GetMethodID": {
                ...
                if (!advisor.shouldIgnoreJniMethodLookup(lazyValue(clazz), lazyValue(name), lazyValue(signature), callerClassLazyValue)) {
                    config.getOrCreateType(predicate, declaringClassOrClazz).addMethod(name, signature, memberKind);
                    if (!declaringClassOrClazz.equals(clazz)) {
                        config.getOrCreateType(predicate, clazz);
                    }
                }
                break;
            }
            ...
            case "GetStaticFieldID": {
                expectSize(args, 2);
                String name = (String) args.get(0);
                config.getOrCreateType(predicate, declaringClassOrClazz).addField(name, memberKind, false);
                if (!declaringClassOrClazz.equals(clazz)) {
                    config.getOrCreateType(predicate, clazz);
                }
                break;
            ...
            case "NewObjectArray": {
                ...
                config.getOrCreateType(predicate, arrayQualifiedJavaName);
                break;
            }
        }
    }
