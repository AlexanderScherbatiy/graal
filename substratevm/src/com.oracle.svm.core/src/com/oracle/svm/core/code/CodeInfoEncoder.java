...
public class CodeInfoEncoder {
    ...
    /**
     * Encodes annotations attached to a method or parameter in a format based on the one used
     * internally by the JDK ({@link sun.reflect.annotation.AnnotationParser}). The format we use
     * differs from that one on a few points, based on the fact that the JDK encoding is based on
     * constant pool indices, which are not available in that form at runtime.
     * 
