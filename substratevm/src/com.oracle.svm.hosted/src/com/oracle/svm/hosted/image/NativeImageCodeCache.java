...
public abstract class NativeImageCodeCache {
    ...
    public void buildRuntimeMetadata(CFunctionPointer firstMethod, UnsignedWord codeSize) {
        ...
        if (SubstrateOptions.ConfigureReflectionMetadata.getValue()) {
            for (Executable queriedMethod : ImageSingletons.lookup(RuntimeReflectionSupport.class).getQueriedMethods()) {
                System.out.println("Trying to add " + queriedMethod);
                HostedMethod method = imageHeap.getMetaAccess().optionalLookupJavaMethod(queriedMethod);
                if (method != null) {
                    System.out.println("Adding metadata for " + method);
